...
namespace DecisionTrees
{
    class TrainingController
    {
        ...
        public void importExamples(string filepath)
        {
            ...
            while (!reader.EndOfStream)
            {
                ...
                if (row != 1)
                {
                    ...
                    {
                        Console.WriteLine($"Adding {value} to {all_attributes[column]}");
                ...
                else
                {
                    ...
                    foreach (string value in values)
                    {
                        ...
                        if (column == values.Length - 1)
                        {
                            ...
                            Console.WriteLine($"Classifier identified as {value}");
                            this.target = value;
                        }
                        else
                        {
                            // Add it to the columns-that-are-not-the-classifier list.
                            Console.WriteLine($"Column {value} found.");
